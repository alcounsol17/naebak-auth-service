...
class TokenModelsTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="tokenuser", email="token@example.com", password="password123"
        )
    ...
    def test_refresh_token_model(self):
        ...
        refresh_token = RefreshToken.objects.create(
            user=self.user, token="testrefreshtoken", expires_at=expires_at
        )
    ...
    def test_password_reset_token_model(self):
        ...
        reset_token = PasswordResetToken.objects.create(
            user=self.user, expires_at=expires_at
        )
